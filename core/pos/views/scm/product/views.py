...
from core.pos.models import PurchaseDetail
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            if action == 'search':
                ...
                data = []
                for detail in PurchaseDetail.objects.filter(product_id=request.POST['id']).exclude(state=False):
                    item = detail.toJSON()
    ...
    def validate_data(self):
        ...
        try:
            ...
            if type == 'name':
                ...
                code = self.request.POST['name'].strip()
                if Product.objects.filter(code__exact=code):
                    data['valid'] = False
        except:
            pass
    ...
    def validate_data(self):
        ...
        try:
            ...
            if type == 'name':
                ...
                code = self.request.POST['code'].strip()
                if Product.objects.filter(code__iexact=code).exclude(id=id):
                    data['valid'] = False
        except:
            pass
        return JsonResponse(data)
