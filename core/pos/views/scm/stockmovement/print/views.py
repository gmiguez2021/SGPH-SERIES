...
from core.pos.models import PurchaseRequest, Company, StockMovement


class StockPrintMovementView(LoginRequiredMixin, View):
    ...
    def get_height_ticket(self):
        sale = StockMovement.objects.get(pk=self.kwargs['pk'])
        height = 120
        increment = sale.stockmovementdetail_set.all().count() * 5.45
        height += increment
        return round(height)
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            template = get_template('scm/stockmovement/print/request.html')
