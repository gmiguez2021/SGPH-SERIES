    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            if action == 'search':
                ...
                data = []
                for detail in PurchaseDetail.objects.filter(purchase_id=request.POST['id']):
        ...
        try:
            if action == 'add':
                with transaction.atomic():
                    ...
                    purchase.sucursal_id = int(request.POST['sucursal'])
                    ...
                    for p in json.loads(request.POST['products']):
                        ...
                        for s in p['series']:
                            ...
                            detail.sucursal_id = purchase.sucursal_id
                            detail.cant = 1
                            detail.serie = s
                            detail.price = float(p['price'])
                ...
                data = []
                term = request.POST['term']
                for p in Provider.objects.filter(Q(name__icontains=term) | Q(ruc__icontains=term)).order_by('name')[0:10]:
                    item = p.toJSON()
                ...
                while PurchaseDetail.objects.filter(serie=serie, state=True).exists():
                    serie = self.generate_serie()
                data['serie'] = serie
                ...
                if code in series:
                    ...
                    data['valid'] = not PurchaseDetail.objects.filter(serie=code).exclude(serie__in=series).exists()
            else:
                data['error'] = 'No ha ingresado una opción'
        except Exception as e:
            data['error'] = str(e)
